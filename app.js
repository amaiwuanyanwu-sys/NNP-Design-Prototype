(function () {
  ...
  function renderNutriRail() {
    ...
    /* a gap keeps one decimal only when it has one, so "6.7 g" but "135 g" */
    var fmtDelta = function (v) {
      var r = Math.round(v * 10) / 10;
      return (r % 1 ? r.toFixed(1) : String(Math.round(r))).replace(/\B(?=(\d{3})+(?!\d))/, ',');
    };
    ...
    /* Metric widgets: Calories / Carbs / Fat / Protein, each against its own target
       when one is set. `over` is the deeper shade the overshoot stub uses. */
    var metrics = [
      { label: 'Calories', value: t.kcal, target: savedPrefs.calories, unit: 'kcal', color: 'var(--n600)', over: 'var(--n800)' },
      { label: 'Carbs', value: t.carbs, target: savedPrefs.grams.carbs, unit: 'g', color: 'var(--lake600)', over: 'var(--lake200)' },
      { label: 'Fat', value: t.fat, target: savedPrefs.grams.fat, unit: 'g', color: 'var(--creamsicle200)', over: 'var(--creamsicle400)' },
      { label: 'Protein', value: t.protein, target: savedPrefs.grams.protein, unit: 'g', color: 'var(--spring400)', over: 'var(--spring800)' }
    ];
    document.getElementById('metricGrid').innerHTML = metrics.map(function (m) {
      ...
      /* Progress reads as a signed gap to the target rather than a percentage.
         Anything within half a unit of the target counts as on target, so a
         rounded display value never contradicts the label beside it. */
      ...
      if (onTarget) {
        ...
        desc = fmtDelta(Math.abs(diff)) + ' ' + m.unit + (over ? ' over' : ' under');
        descIcon = over ? 'arrow-upward' : 'arrow-downward';
      }

      /* At or past the target the bar is full, split into the part that reached
         the target and an overshoot stub in a deeper shade of the same hue. */
      var bar;
      if (diff >= 0 && m.target) {
        bar = '<div class="metric-bar-fill grow" style="background:' + m.color + ';"></div>' +
              (over ? '<div class="metric-bar-over" style="background:' + m.over + ';"></div>' : '');
      } else {
        var fillPct = m.target ? Math.max(Math.min(m.value / m.target, 1) * 100, 0) : 0;
        bar = '<div class="metric-bar-fill" style="width:' + fillPct + '%; background:' + m.color + ';"></div>';
      }

      return '<div class="metric-card">' +
        '<div class="metric-head"><span class="metric-name">' + m.label + '</span>' +
          '<span class="metric-info" title="' + m.label + ' against the daily target">' + ico('information', 16) + '</span>' +
        '</div>' +
